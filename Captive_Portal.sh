...
echo  "[*]LET'S PROCEED"
echo  "[*]Lets Download the required softwares for the operation first "

	apt-get update
	apt-get install dnsmasq
	apt-get install hostapd

echo "[*]Now the softwares have been downloaded "
...
echo "[*]Enabling hostapd server "

	hostapd /root/Downloads/fake-ap/hostapd.conf -B

echo "[*]and now starting apache2 web server "

	service apache2 start 

echo "[*]Now wait for a while let the clients connect "
